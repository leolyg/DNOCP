...
if __name__ == "__main__":
    ...
    parser.add_argument('--size', default=8, help="size of TSP")
    ...
    parser.add_argument('--gpu', action='store_true',
                        default=False, help="learning rate")
    ...
    save_root = './output/gpn_tsp'+str(size)+'.pt'
    gpu = bool(args['gpu'])
    ...
    print('=========================')
    if gpu:
        model = GPN(n_feature=2, n_hidden=128, use_cuda=True).cuda()
    else:
        model = GPN(n_feature=2, n_hidden=128)
    # load model
    ...
    for epoch in range(n_epoch):
        for i in tqdm(range(steps)):
            ...
            X = torch.Tensor(X) # X.shape -> if B = 2, (2, 8, 2)
            if gpu:
                X.cuda()
            ...
            for k in range(size):
                ...
                sampler = torch.distributions.Categorical(output) # sampler -> Categorical(probs: torch.Size([512, 8]))
                idx = sampler.sample()         # now the idx has B elements, if B = 2  idx -> tensor([4, 6])

                Y1 = Y[[i for i in range(B)], idx.data].clone() #  Y1 -> tensor([[0.3586, 0.2366], [0.1037, 0.3007]]
                print(Y1)
                if k == 0:
                    Y_ini = Y1.clone()
                if k > 0:
                ...
                TINY = 1e-15
                logprobs += torch.log(output[[i for i in range(B)],
                                      idx.data]+TINY)
            ...
            if i % 50 == 0:
                ...
                for k in range(size):
                    ...
                    output, h, c, hidden_u = model(
                        x=x, X_all=X, h=h, c=c, mask=mask)
