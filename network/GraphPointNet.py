...
class Attention(nn.Module):
    ...
    def forward(self, q, ref):       # query and reference
        ...
        v_view = self.v.unsqueeze(0).expand(
            self.batch_size, self.dim).unsqueeze(2)
...
class GPN(torch.nn.Module):
    ...
    def __init__(self, n_feature, n_hidden, use_cuda=False):
        ...
        global USE_CUDA
        if use_cuda:
            USE_CUDA = True # USE_CUDA defalut is False.
        # lstm for first turn
    ...
    def forward(self,X):
        ...
        ###预测的每一步
        for k in range(size):            
            output, h, c, _ =self.one_step(x=x, X_all=X, h=h, c=c, mask=mask)
        ...
        # predicted_probability = predicted_probability.contiguous().view(-1, predicted_probability.size()[-1])
        pointer = torch.argmax(predicted_probability,dim=2)
        return predicted_probability,pointer
        

    def one_step(self, x, X_all, mask, h=None, c=None, latent=None):
        '''
